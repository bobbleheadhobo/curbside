...
log = logging.getLogger("curbside.cli")
...
def cmd_once(args) -> int:
    ...
    #
    # Only `--due` is gated. A bare `curbside once` is a person at a keyboard
...
def cmd_seed_demo(args) -> int:
    ...
    out.write_text("# Generated by `curbside seed-demo`. Offline: fixture source,\n"
                   "# stub scorer, notifications off. Safe to delete.\n" + text)

    print(f"built {args.db}")
    print("  " + " · ".join(f"{k} {v}" for k, v in sorted(counts.items())))
    print(f"\n  serve it:\n"
          f"    .venv/bin/python -m curbside.cli --config {out.name} serve --port 8478")
    store.close()
    return 0
